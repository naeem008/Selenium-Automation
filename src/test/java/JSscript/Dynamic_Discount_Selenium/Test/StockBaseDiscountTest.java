...
import DataProvider.StockBaseDataProvider;
...
public class StockBaseDiscountTest {
	...
	@Test(dataProvider = "StockBaseData", dataProviderClass = StockBaseDataProvider.class)
	public void TestStockbase(List<StockBaseDTO> stock) throws InterruptedException {
		for (StockBaseDTO stockD : stock) {
			...
			driver.findElement(By.xpath(GlobalXpath.DISCOUNT_INPUT)).sendKeys("Stock Base Discount");
			...
			select.selectByVisibleText("Stock Base Discount");
			...
			// Select Product 1
			searchBox.sendKeys(stockD.getProduct1());
			System.out.println("Get product1 name from the excel");

			// Wait for the result to be present and then click it
			WebElement productOption = wait11.until(
					ExpectedConditions.elementToBeClickable(By.xpath("//div[text()='" + stockD.getProduct1() + "']")));
			System.out.println("Product option found and clicked.");
			productOption.click();
			// Select Product 2
			searchBox.sendKeys(stockD.getProduct2());

			System.out.println("Get product2 name from the excel");

			// Wait for the result to be present and then click it
			WebElement productOption1 = wait11.until(
					ExpectedConditions.elementToBeClickable(By.xpath("//div[text()='" + stockD.getProduct2() + "']")));
			System.out.println("Product option found and clicked.");
			...
			//
			driver.findElement(By.xpath(GlobalXpath.Bulk_Title)).sendKeys("Stock Base Discount");
			...
			searchproduct.sendKeys(stockD.getProduct1());
			...
			searchproduct1.sendKeys(stockD.getProduct2());
